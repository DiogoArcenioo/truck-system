...
import { ListarCombustivelDto } from './dto/listar-combustivel.dto';

type RegistroBanco = Record<string, unknown>;

type MapaColunasCombustivel = {
...
export class CombustiveisService {
  ...
  async listarTodos(idEmpresa: number) {
    return this.executarComRls(idEmpresa, async (manager, colunas) => {
      ...
      const sql = `
        SELECT *
        FROM app.combustiveis
        ${filtros.length > 0 ? `WHERE ${filtros.join(' AND ')}` : ''}
        ORDER BY ${this.quote(colunas.descricao)} ASC, ${this.quote(colunas.idCombustivel)} ASC
      `;

      const rows = await manager.query(sql, valores);
      const combustiveis = rows.map((row) => this.mapear(row, colunas));
  ...
  async cadastrar(
    ...
    try {
      return await this.executarComRls(idEmpresa, async (manager, colunas) => {
        ...
        const sql = `
          INSERT INTO app.combustiveis (${insertCols.join(', ')})
        ...
        if (!registro) {
          throw new BadRequestException('Falha ao cadastrar combustivel.');
        }

        return {
          sucesso: true,
          mensagem: 'Combustivel cadastrado com sucesso.',
          combustivel: this.mapear(registro, colunas),
        };
      });
    } catch (error) {
  ...
  async buscarPorId(idEmpresa: number, idCombustivel: number) {
    return this.executarComRls(idEmpresa, async (manager, colunas) => {
      const filtros: string[] = [];
      const valores: Array<string | number> = [];

      valores.push(idCombustivel);
      filtros.push(`${this.quote(colunas.idCombustivel)} = $${valores.length}`);
      ...
      if (!registro) {
        throw new NotFoundException(
          'Combustivel nao encontrado para a empresa logada.',
        );
      }
  ...
  async atualizar(
    ...
    try {
      return await this.executarComRls(idEmpresa, async (manager, colunas) => {
        ...
        const registro = rows[0];
        if (!registro) {
          throw new NotFoundException('Combustivel nao encontrado para a empresa logada.');
        }

        return {
          sucesso: true,
          mensagem: status === 'I' ? 'Combustivel inativado com sucesso.' : 'Combustivel atualizado com sucesso.',
          combustivel: this.mapear(registro, colunas),
        };
      });
    } catch (error) {
  ...
  private async executarComRls<T>(
    ...
    callback: (
      ...
      colunas: MapaColunasCombustivel,
    ) => Promise<T>,
  ): Promise<T> {
  ...
  private async carregarMapaColunas(
    manager: EntityManager,
  ): Promise<MapaColunasCombustivel> {
    const rows = await manager.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = 'app'
        AND table_name = 'combustiveis'
    `);

    if (rows.length === 0) {
      throw new BadRequestException('Tabela app.combustiveis nao encontrada.');
    }
    ...
    return {
      idCombustivel: this.encontrarColuna(
        set,
        ['id_combustivel', 'id_combustiveis', 'id'],
        'id do combustivel',
      )!,
      descricao: this.encontrarColuna(
        set,
        ['descricao', 'combustivel', 'nome'],
  ...
  private encontrarColuna(
    ...
    if (obrigatoria) {
      throw new BadRequestException(
        `Estrutura da tabela app.combustiveis invalida: coluna de ${descricao} nao encontrada.`,
      );
    }
  ...
  private mapear(
    ...
    colunas: MapaColunasCombustivel,
  ): ListarCombustivelDto {
    return {
      idCombustivel: this.converterNumero(registro[colunas.idCombustivel]) ?? 0,
  ...
  private async validarDuplicidadeDescricao(
    ...
    if (idCombustivelIgnorado !== undefined) {
      valores.push(idCombustivelIgnorado);
      filtros.push(`${this.quote(colunas.idCombustivel)} <> $${valores.length}`);
    }
    ...
    if (rows[0]) {
      throw new BadRequestException(
        'Ja existe um combustivel com essa descricao para a empresa logada.',
      );
    }
  }
  ...
  private normalizarDescricao(valor: string) {
    ...
    if (!texto) {
      throw new BadRequestException('Descricao do combustivel e obrigatoria.');
    }
    return texto;
  }
  ...
  private tratarErroPersistencia(error: unknown): never {
    ...
    throw new BadRequestException(
      'Nao foi possivel cadastrar combustivel neste momento.',
    );
  }
}
