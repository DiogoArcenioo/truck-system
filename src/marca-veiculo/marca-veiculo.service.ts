...
export class MarcaVeiculoService {
  ...
  async cadastrar(
    ...
    try {
      return await this.executarComRls(idEmpresa, async (manager, colunas) => {
        ...
        const sql = `
          INSERT INTO app.marca_vei (${insertCols.join(', ')})
        ...
        if (!registro) {
          throw new BadRequestException('Falha ao cadastrar marca de veiculo.');
        }

        return {
          sucesso: true,
          mensagem: 'Marca de veiculo cadastrada com sucesso.',
  ...
  private async validarDuplicidadeDescricao(
    ...
    if (rows[0]) {
      throw new BadRequestException(
        'Ja existe uma marca de veiculo com essa descricao para a empresa logada.',
      );
    }
  }
  ...
  private normalizarDescricao(valor: string) {
    ...
    if (!texto) {
      throw new BadRequestException('Descricao da marca e obrigatoria.');
    }
    return texto;
  }
  ...
  private tratarErroPersistencia(error: unknown): never {
    ...
    throw new BadRequestException(
      'Nao foi possivel cadastrar marca de veiculo neste momento.',
    );
  }
}
